...
export SCRIPTPATH=$PWD
               ...
               cd ../Externals/LibXML &&
               sh ./autogen.sh CFLAGS="-m32 -fPIC" CXXFLAGS="-m32 -fPIC" LDFLAGS="-m32 -fPIC" --disable-shared --enable-static --without-iconv --prefix=${DIR} &&
               ( (make  CFLAGS="-m32 -fPIC" CXXFLAGS="-m32 -fPIC" LDFLAGS="-m32 -fPIC"  $NUM_PROCS && make install) || cp .libs/lib* ${DIR}/lib/) &&
               cd ../.. &&
               cd Externals/pcre/scripts &&
               ruby $SCRIPTPATH/vcproj2cmake.rb pcre.vcproj &&
               cmake . -DCMAKE_CXX_COMPILER=`which g++` -DCMAKE_C_COMPILER=`which gcc` -DCMAKE_CXX_FLAGS="-m32 -fPIC  -g2" -DCMAKE_C_FLAGS="-m32 -fPIC  -g2" -DCMAKE_SHARED_LINKER_FLAGS="-m32 -fPIC" -DCMAKE_MODULE_LINKER_FLAGS="-m32 -fPIC" &&
               make $NUM_PROCS &&
               cd ../../.. &&
